...
| Tool | Vendor | Open Source | Pricing (input, $/M tokens)¹ | GUI | Flexibility³ | Deployment⁴ | Typical use cases |
|---|---|---|---|---|---|---|---|
| **[Claude Code CLI](https://docs.claude.com/claude-code)** | Anthropic | ❌ Closed (binary) | Sonnet 4.5: **$3** · Opus 4.x: **$15** · Haiku 4.5: **$1** | Terminal only (official VS Code extension) | 🟡 Medium — Anthropic models only; extensible via Skills + MCP servers; sub-agents supported | 🟢 Low — `npm i -g @anthropic-ai/claude-code` + Anthropic / Bedrock / Vertex login | Long-form coding agent; multi-file refactor; PR review; doc/paper revision; codebase exploration |
| **[Codex CLI](https://github.com/openai/codex)** | OpenAI | ✅ MIT | GPT-5-Codex: **~$1.25** · GPT-5.3-Codex: **~$1.75** | Terminal only (also web + IDE versions) | 🟡 Medium — OpenAI models only; open-source code editable; MCP support | 🟢 Low — `npm i -g @openai/codex` + `OPENAI_API_KEY` | Scripted code edits; shell automation; quick one-shot tasks; cheap iteration |
| **[Antigravity CLI](https://ai.google.dev/gemini-api/docs/antigravity-agent)** | Google | ❌ Closed (preview) | Gemini 3 Pro: **~$2** · Gemini 3 Flash: **$0.50** | Has GUI (Antigravity IDE / agent workspace) | 🔴 Low — Google models only; closed preview; limited 3rd-party extension surface | 🟡 Medium — download binary + Google account + preview access; sandboxed managed agents | Agent-first IDE workflows; managed long-running tasks; teams already in Google ecosystem |
| **[OpenClaw](https://github.com/openclaw/openclaw)** | Community (openclaw.ai) | ✅ MIT | BYOK — depends on chosen model² | Web UI + multi-channel (Telegram / Discord / Slack / WhatsApp / iMessage / …) | 🟢 High — model-agnostic (Anthropic / OpenAI / Google / Bedrock / OpenRouter / Ollama / …); skills + plugins + MCP; pluggable channels; cron + multi-agent orchestration | 🔴 Higher — `npm i -g openclaw` + config YAML + channel webhooks + skills setup | 24/7 personal assistant; messenger-bound bot; cross-device automation (phone/laptop/server); scheduled cron jobs; group-chat orchestration; long-horizon research workflows |
...
⁴ "Deployment" reflects setup effort to reach a first working session, including auth, runtime install, and any required external service wiring (channels, webhooks, sandboxes).
